...
// =======================================================
// 🔍 SEARCH SWEETS (THIS WAS MISSING — TDD GREEN STEP)
// =======================================================
...
const filtered = allSweets.filter((sweet: any) => {
  if (name && !sweet.name.toLowerCase().includes(String(name).toLowerCase())) {
    return false;
  }
  ...
  if (minPrice && sweet.price < Number(minPrice)) {
    return false;
  }

  if (maxPrice && sweet.price > Number(maxPrice)) {
    return false;
  }
